...
/**
 * 弹窗文案：知道余额就把余额念出来（「当前余额 0 点」比「余额不足」更能让用户明白
 * 自己不是被误拦），不知道就只说不足 —— 绝不编一个数字。
 */
export function pointsInsufficientContent(name?: string, points?: number): string {
  const label = String(name || '').trim();
  const who = label ? `「${label}」` : '该内容';
  const balance = Number.isFinite(Number(points)) ? Number(points) : null;
  if (balance !== null && balance <= 0) {
    return `${who}每次调用都会消耗算力，当前余额 ${balance} 点，已不足以继续。充值后即可继续使用。`;
  }
  return `${who}需要消耗算力，当前余额不足。充值后即可继续使用。`;
}

/**
 * 弹窗引导去充值。
 *
 * @param options.name   智能体 / 工作流名称，用于把「是哪一个」说清楚；不传则用「该内容」
 * @param options.points 已知余额；`<= 0` 时文案会把它念出来，未知则不念
 * @returns true = 用户选择去充值（已跳转）；false = 用户放弃（或已有弹窗在飞）
 */
export async function promptRecharge(options: { name?: string; points?: number } = {}): Promise<boolean> {
  if (inFlightPrompt) return inFlightPrompt;
  const task = (async () => {
    const res = await Taro.showModal({
      title: '算力不足',
      content: pointsInsufficientContent(options.name, options.points),
      confirmText: '去充值',
      cancelText: '再想想',
      confirmColor: '#305CE0',
    });
 ...
 * 余额明确为 0（或负数）时弹窗引导充值。
 *
 * ⚠️ 与 ensureVipAccess 一样**失败即放行**：拿不到档案（未登录 / 网络抖动）时不在客户端拦，
 * 交给服务端 402 给出准确结果；`points` 缺失（老服务端）同样放行。
 *
 * @param item    智能体 / 工作流（只用它的 name 做文案）
 * @param profile 已有档案时直接传入，省掉一次 getMe()
 * @returns true = 放行；false = 已拦截（弹窗已展示或已跳转充值页）
 */
export async function ensureEnoughPoints(
  ...
  await promptRecharge({ name: item?.name, points });
  return false;
}
