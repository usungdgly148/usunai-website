...
import { Plus, Trash2, Edit2, Check, X, ArrowUp, ArrowDown, Tag, Sidebar as SidebarIcon, Bookmark, Home as HomeIcon, Eye, EyeOff, Layers } from 'lucide-react';
...
const EMPTY_DRAFT = { name: '', group: '', icon: 'LayoutGrid', color: 'bg-blue-50 text-blue-600', sortOrder: '' };
...
export default function AdminCategories() {
  ...
  const startEdit = (cat) => {
    setEditing(cat.id);
    setDraft({ name: cat.name, group: cat.group || '', icon: cat.icon, color: cat.color, sortOrder: cat.sortOrder });
  };
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="分类管理"
        subtitle="配置前台左侧导航与首页标签，可自由增删改、排序，并控制每个分类在前台各位置的展示"
        actions={
